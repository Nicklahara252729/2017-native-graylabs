...
$gl_sql_report = mysql_query("select * from gl_kasir join gl_pasok on (gl_kasir.id_kasir=gl_pasok.id_kasir) join gl_detailpasok on (gl_pasok.id_pasok=gl_detailpasok.id_pasok) join gl_buku on (gl_detailpasok.id_buku=gl_buku.id_buku) where gl_kasir.id_kasir='$gl_get_id_pasok'");
$gl_sql_report3 = mysql_query("select * from gl_kasir join gl_penjualan on (gl_kasir.id_kasir=gl_penjualan.id_kasir) join gl_detailjual on (gl_penjualan.id_penjualan=gl_detailjual.id_penjualan) join gl_buku on (gl_detailjual.id_buku=gl_buku.id_buku) where gl_kasir.id_kasir='$gl_get_id_pasok'");
$gl_sql_report2 = mysql_query("select * from gl_kasir join gl_pasok on (gl_kasir.id_kasir=gl_pasok.id_kasir) join gl_detailpasok on (gl_pasok.id_pasok=gl_detailpasok.id_pasok) where gl_kasir.id_kasir='$gl_get_id_pasok'");
}else{
    $gl_sql_report = mysql_query("select * from gl_kasir join gl_pasok on (gl_kasir.id_kasir=gl_pasok.id_kasir) join gl_detailpasok on (gl_pasok.id_pasok=gl_detailpasok.id_pasok) join gl_buku on (gl_detailpasok.id_buku=gl_buku.id_buku) where gl_kasir.id_kasir=''");
$gl_sql_report3 = mysql_query("select * from gl_kasir join gl_pembelian on (gl_pembelian.id_kasir=gl_pembelian.id_kasir) join gl_detailbeli on (gl_pasok.id_pasok=gl_detailpasok.id_pasok) join gl_buku on (gl_detailbeli.id_buku=gl_buku.id_buku) where gl_kasir.id_kasir=''");
$gl_sql_report2 = mysql_query("select * from gl_kasir join gl_pasok on (gl_kasir.id_kasir=gl_pasok.id_kasir) join gl_detailpasok on (gl_pasok.id_pasok=gl_detailpasok.id_pasok) where gl_kasir.id_kasir=''");
}
...
<html>
    <head>
        ...
        <title>Graylabs - Laporan Perkasir</title>
        <script>
            var i=1;
            function slide(){
                if(i==1){
                    ...
                    <div class="col-md-12" id="content-full2">
                        <div class="rows">
                         ...
                         <div class="report-logo"></div></div>
                                    ...
                                    <div class="col-md-4" id="search2" align="center">
                                        <h4>GRAYLABS BOOK STORE<br>LAPORAN PER KASIR<br><span class="small">Jl.Patriot No 11 KM 7.5 MEDAN - SUMATRA UTARA</span></h4>
                                    </div>
                                    <div class="col-md-4">
                        ...
                        <div class="rows">
                            <div class="col-md-12">
                                ...
                                <?php echo $gl_r['akses']; ?>
                                    <button onclick="window.print();" style="float:right;" type="button" class="btn btn-purple tip" data-toggle="tooltip" data-placement="bottom" title="Print"><span class="glyphicon glyphicon-print" ></span></button>
                                </div>
                            </div>
                        </div>
                        <div class="rows">
                            <div class="col-md-6" id="report-table">
                                <div class="rows">
                                <table class="table table-striped" style="box-shadow:0px 0px 0px 0px;border:solid 1px lightgray;">
                                    ...
                                    if($gl_rows > 0){
                                        ...
                                        while($gl_fetch = mysql_fetch_array($gl_sql_report)){
                                            $gl_total = $gl_fetch['total'] + $gl_total;
                                     
                                    ?>
                                    <tr>
                                        <td><?php echo $gl_no; ?></td>
                                        <td><?php echo $gl_fetch['id_pasok']; ?></td>
                                        <td><?php echo $gl_fetch['tanggal']; ?></td>
                                        <td>Pembelian</td>
                                        
                                        <td align="center"><?php echo $gl_fetch['jml_beli']; ?> </td>
                                        <td align="right"><?php
                                            echo"Rp ".number_format($gl_fetch['harga_pokok'],0,',','.'); ?></td>
                                        <td align="right" ><?php echo"Rp ".number_format($gl_fetch['total'],0,',','.'); ?> </td>
                                    ...
                                    ?>
                                    <tr>
                                        <td colspan="4" align="center" style="border-right:solid 1px lightgray;">TOTAL SELURUH PEMBELIAN</td>
                                        <td align="right" colspan="3"><b><?php echo"Rp ".number_format($gl_total,0,',','.');  ?></b></td>
                                    </tr>
                                </table>
                                    </div>
                            </div>
                            <div class="col-md-6" id="report-table">
                                <table class="table table-striped" style="box-shadow:0px 0px 0px 0px;border:solid 1px lightgray;">
                                    ...
                                    <?php
                                    $gl_rows3 = mysql_num_rows($gl_sql_report3);
                                    $gl_total3 = 0;
                                    if($gl_rows3 > 0){
                                        $gl_no3 =1;
                                        
                                        while($gl_fetch3 = mysql_fetch_array($gl_sql_report3)){
                                        
                                            $gl_total3 = $gl_fetch3['jumlah']+ $gl_total3;
                                     
                                    ?>
                                    <tr>
                                        <td><?php echo $gl_no3; ?></td>
                                        <td><?php echo $gl_fetch3['id_penjualan']; ?></td>
                                        <td><?php echo $gl_fetch3['tanggal']; ?></td>
                                        <td>Terjual</td>
                                        
                                        <td align="center"><?php echo $gl_fetch3['jml_beli']; ?> </td>
                                        <td align="right"><?php
                                            $gl_diskon3 = $gl_fetch3['harga_jual']*$gl_fetch3['diskon']/100;
                                            $gl_ppn3   = $gl_fetch3['harga_jual']*$gl_fetch3['ppn']/100;
                                            echo"Rp ".number_format(($gl_fetch3['harga_jual']-$gl_diskon3)+$gl_ppn3,0,',','.'); ?></td>
                                        <td align="right" ><?php echo"Rp ".number_format($gl_fetch3['total_peritem'],0,',','.'); ?> </td>
                                    </tr>
                                    <?php
                                            $gl_no++;
                                       }
                                    ...
                                    ?>
                                    <tr>
                                        <td colspan="4" align="center" style="border-right:solid 1px lightgray;">TOTAL SELURUH PENJUALAN</td>
                                        <td colspan="3" align="right"><b><?php echo"Rp ".number_format($gl_total3,0,',','.');  ?></b></td>
                                    </tr>
                                </table>
                            </div>
                </div>
                        <div class="rows">
                            <div class="col-md-6" id="pad-bottom">
                                Manager :<br>
                                <b><?php
                                $gl_manager = mysql_query("select * from gl_kasir where akses='manager' or akses='superadmin'");
                                     $gl_f_man = mysql_fetch_array($gl_manager);
                                    echo $gl_f_man['nama'];
                                ?></b><br><br>
                                ...............................
                            </div>
