...
$gl_sql_count = mysql_query("select count(id_penjualan) as akhir from gl_penjualan");
$gl_sql_row= mysql_fetch_array($gl_sql_count, MYSQL_ASSOC);
$gl_num = $gl_sql_row['akhir'];
$gl_number = $gl_num+1; 
switch(strlen($gl_number)){
    case 1 : $gl_kode = "TE0000".$gl_number; break;
    case 2 : $gl_kode = "TE000".$gl_number; break;
    case 3 : $gl_kode = "TE00".$gl_number; break;
    case 4 : $gl_kode = "TE0".$gl_number; break;
...
if(isset($_GET['reset'])){
    ...
    header("location:config_transaksi_eksport.php");
}
...
<html>
    <head>
        ...
        <title>Graylabs - Penjualan Buku</title>
        <script>
            var i=1;
            function slide(){
                if(i==1){
    ...
    <body>
        ...
        <div class="full">
            <div class="container-fluid">
                <div class="rows">
                    <div class="col-md-12" id="content-full">
                        ...
                        <div class="rows">
                    <div class="col-md-5" style="padding-bottom:20px;border:solid 1px lightgray;margin-bottom:10px;height:350px;">
                        <form target="_self" enctype="multipart/form-data" name="import" id="import" method="post">
                            ...
                            <div class="col-md-12" id="pad-top">
                                ...
                                <div class="col-md-7">
                                                      <input type="text" name="gl_kasir" id="id_kasir" value="<?php echo $_SESSION['nama']; ?>" readonly class="col-md-12">
                                </div>
                            </div>
                            <div class="col-md-12" id="pad-top">
                                <div class="col-md-1">
                                ...
                                <div class="col-md-7">
                                                      <input type="text" name="gl_konsumen" id="gl_konsumen" class="col-md-12" placeholder="Nama Konsumen">
                                </div>
                            </div>
                            <div class="col-md-12" id="pad-top">
                                <div class="col-md-1">
			...
			$gl_awal=1;$gl_sub=0;$gl_total=0;
                ...
                @$gl_konsumen = strip_tags(trim($_POST['gl_konsumen']));
                ...
                @$gl_tampil = mysql_fetch_array(mysql_query("select * from gl_buku where id_buku= '$gl_id_buku'"));
				@$gl_harga_jual=@$gl_tampil['harga_jual'];
				@$gl_diskon=@$gl_tampil['diskon'];
				@$gl_ppn=@$gl_tampil['ppn'];
			...
			if (@$gl_id_buku!=''){
                ...
                @$gl_totdis = $gl_harga_jual * $gl_diskon / 100;
                @$gl_totppn = $gl_harga_jual * $gl_ppn /100;
				@$gl_sub=((($gl_harga_jual - $gl_totdis) + $gl_totppn)*$gl_jmlh);
				$_SESSION["akhir"][$_SESSION["transaksi"]]=array($gl_judul,$gl_harga_jual,$gl_diskon,$gl_ppn,$gl_jmlh,$gl_sub,$gl_id_buku,$gl_tanggal,$gl_konsumen,$gl_kode);
                header("location:config_transaksi_eksport.php");
			}
                            ?>
				...
				if (@$_SESSION["akhir"][$i][0]!=''){ ?>
							...
							<td>
                                <?php echo $i ?>
                                <input type="hidden" name="gl_x_kode" value="<?php echo @$_SESSION['akhir'][$i][9]?>">
							...
							<td><?php echo"Rp ".number_format(@$_SESSION['akhir'][$i][1],0,',','.') ?></td>
                                <td></td>
							<td><?php echo @$_SESSION['akhir'][$i][2] ?> %</td>
							<td><?php echo @$_SESSION['akhir'][$i][3] ?> %</td>
							<td align="center">
                                <?php echo @$_SESSION['akhir'][$i][4] ?>
                                <input type="hidden" name="gl_x_jml" value="<?php echo @$_SESSION['akhir'][$i][4] ?>">
                                </td>
							<td align="right">
                <?php echo"Rp ".number_format(@$_SESSION['akhir'][$i][5],0,',','.'); ?>
                <input type="hidden" name="gl_x_totperitem" value="<?php echo @$_SESSION['akhir'][$i][5]; ?>">
                                </td>
					</tr>
                            <?php
                                                   }
                    $gl_total=@$_SESSION['akhir'][$i][5]+$gl_total;
			...
			?>
                            ...
                            <tr>
                                <td colspan="8" align="right" id="paid">
                                    <button type="button" onclick="location.href='config_transaksi_eksport.php?reset'" name="reset" id="reset" class="btn btn-danger">Reset</button>
                                    <?php
if(isset($_POST['gl_x_kode'])){
    ...
    $gl_sql_det = mysql_query("insert into gl_penjualan set id_penjualan='$gl_x_kode2', id_kasir='$_SESSION[id_kasir]',jumlah='$gl_x_total2',tanggal='$gl_x_tanggal2'");
    $j=0;
    $gl_awal=$_SESSION["transaksi"];
	while($j <= $gl_awal){
        $gl_x_kode = @$_SESSION['akhir'][$j][9];
        $gl_x_kasir = $_SESSION['nama'];
        $gl_x_id_buku = @$_SESSION['akhir'][$j][6];
        $gl_x_tanggal =@$_SESSION['akhir'][$j][7] ;
        $gl_x_konsumen = @$_SESSION['akhir'][$j][8];
        $gl_x_jml = @$_SESSION['akhir'][$j][4];
        $gl_x_totperitem = @$_SESSION['akhir'][$j][5];
        $gl_x_total = strip_tags(trim($_POST['gl_x_total']));
        if($gl_x_kode!="" and $gl_x_id_buku!="" and $gl_x_jml!=""){
			$query = mysql_query("insert into gl_detailjual set id_penjualan='$gl_x_kode',id_buku='$gl_x_id_buku', nama_konsumen='$gl_x_konsumen',jml_beli='$gl_x_jml',total_peritem='$gl_x_totperitem'");
            $gl_sql_updt = mysql_query("update gl_buku set stok=(stok - '$gl_x_jml'), jmljual=(jmljual + '$gl_x_jml') where id_buku='$gl_x_id_buku'");
    ...
    ?>
                                    ...
                                    ?>
                                    <button type="submit" class="btn " id="green">Bayar</button>
                                </td>
                            </tr>
                        </table>
                            </form>
