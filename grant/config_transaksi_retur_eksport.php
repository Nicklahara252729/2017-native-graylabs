...
$gl_sql_count = mysql_query("select count(id_retur) as akhir from gl_retur");
$gl_sql_row= mysql_fetch_array($gl_sql_count, MYSQL_ASSOC);
$gl_num = $gl_sql_row['akhir'];
$gl_number = $gl_num+1; 
switch(strlen($gl_number)){
    case 1 : $gl_kode = "RTR0000".$gl_number; break;
    case 2 : $gl_kode = "RTR000".$gl_number; break;
    case 3 : $gl_kode = "RTR00".$gl_number; break;
    case 4 : $gl_kode = "RTR0".$gl_number; break;
...
if(isset($_GET['sch_cari'])){
    ...
    $gl_sql_cari = mysql_query("select * from gl_transit order by judul asc limit $gl_posisi, $gl_limit");
}else{
...
if(isset($_GET['reset'])){
    ...
    header("location:config_transaksi_retur_eksport.php");
}
...
<html>
    <head>
        ...
        <title>Graylabs - Retur Buku ke Supplier</title>
        <script>
            var i=1;
            function slide(){
                if(i==1){
    ...
    <body>
        ...
        <div class="full">
                         ...
                         <div class="col-md-2 tip" data-toggle="tooltip" data-placement="bottom"  title="Refresh" id="refresh" onclick="window.location.reload()">
                             <span class="glyphicon glyphicon-refresh"></span>
                                    ...
                                    <div class="col-md-4" id="search" align="center">
                                        <h3>TRANSAKSI RETUR KE SUPPLIER</h3>
                                    </div>
                                    <div class="col-md-4">
			...
			$gl_awal=1;$gl_sub=0;$gl_total=0;
                ...
                @$gl_tampil = mysql_fetch_array(mysql_query("select * from gl_buku join gl_detailpasok on (gl_buku.id_buku=gl_detailpasok.id_buku) join gl_distributor on (gl_detailpasok.id_distributor=gl_distributor.id_distributor) where gl_buku.id_buku='$gl_id_buku' order by id_pasok desc"));
                @$gl_id_pasok = @$gl_tampil['id_pasok'];
                @$gl_id_supplier = @$gl_tampil['id_distributor'];
                @$gl_supplier = @$gl_tampil['nama_distributor'];
			...
			if (@$gl_id_buku!=''){
				...
				$_SESSION["akhir"][$_SESSION["transaksi"]]=array($gl_judul,$gl_tanggal,$gl_supplier,$gl_jmlh,$gl_harga_pokok,$gl_sub,$gl_id_buku,$gl_kode,$gl_id_supplier,$gl_id_pasok);
                header("location:config_transaksi_retur_eksport.php");
			}
                            ?>
				...
				if (@$_SESSION["akhir"][$i][0]!=''){ ?>
							...
							<td>
                                <?php echo $i ?>
                                  <input type="hidden" name="gl_x_kode" value="<?php echo @$_SESSION['akhir'][$i][7]?>">
                                </td>
							...
							<td align="right"><?php echo"Rp ".number_format(@$_SESSION['akhir'][$i][4],0,',','.'); ?> </td>
							<td align="right">
                                <?php echo"Rp ".number_format(@$_SESSION['akhir'][$i][5],0,',','.'); ?>
                                <input type="hidden" name="gl_x_jml" value="<?php echo @$_SESSION['akhir'][$i][5] ?>">
                                </td>
			...
			?>
                            ...
                            <tr>
                                <td colspan="7" align="right" id="paid">
                                    <button type="button" onclick="location.href='config_transaksi_retur_eksport.php?reset'" name="reset" id="reset" class="btn btn-danger">Reset</button>
                                    <?php
if(isset($_POST['gl_x_kode'])){
    $j=0;
    $gl_awal=$_SESSION["transaksi"];
	while($j <= $gl_awal){
        $gl_x_kode = @$_SESSION['akhir'][$j][7];
        $gl_x_kasir = $_SESSION['nama'];
        $gl_x_id_buku = @$_SESSION['akhir'][$j][6];
        $gl_x_tanggal =@$_SESSION['akhir'][$j][1] ;
        $gl_x_supplier = @$_SESSION['akhir'][$j][8];
        $gl_x_jmlh = @$_SESSION['akhir'][$j][3];
        $gl_x_pasok = @$_SESSION['akhir'][$j][9];
        $gl_x_total = strip_tags(trim($_POST['gl_x_total']));
        if($gl_x_kode!="" and $gl_x_id_buku!="" and $gl_x_jmlh!=""){
			$query = mysql_query("insert into gl_retur set id_retur='$gl_x_kode',id_distributor='$gl_x_supplier',id_pembelian='$gl_x_pasok', id_buku='$gl_x_id_buku',id_kasir='$_SESSION[id_kasir]',tanggal='$gl_x_tanggal',jumlah='$gl_x_jmlh',total='$gl_x_total'");
            $gl_sql_updt = mysql_query("update gl_buku set stok=(stok-$gl_x_jmlh) where id_buku='$gl_x_id_buku'");
    ...
    ?>
                                    ...
                                    ?>
                                    <button type="submit" class="btn " id="green">Retur</button>
                                </td>
                            </tr>
                        </table>
                            </form>
