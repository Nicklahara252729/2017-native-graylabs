...
if(isset($_GET['reset'])){
    ...
    header("location:config_transaksi_import_short.php?id_buku=$gl_reset");
}
...
<html>
    ...
    $gl_get_idb = $_GET['id_buku'];
                                    $gl_get_sup = mysql_query("select * from gl_distributor join gl_detailpasok on (gl_distributor.id_distributor=gl_detailpasok.id_distributor) where id_buku='$gl_get_idb'");
                                    $gl_fetch_dis = mysql_fetch_array($gl_get_sup)
                                    ?>
                                    <input type="text"  disabled value="<?php echo $gl_fetch_dis['nama_distributor']; ?>">
                                    <input type="hidden" name="gl_supplier" id="gl_supplier" value="<?php echo $gl_fetch_dis['id_distributor']; ?>">
                                    <?php
                                    }
                                        ?>
                                </div>
    ...
    $gl_get_idb = $_GET['id_buku'];
                                    ...
                                    $gl_get_sup = mysql_query("select * from gl_buku where id_buku='$gl_get_idb'");
                                    while($gl_fetch_dis = mysql_fetch_array($gl_get_sup)){
                                    ?>
                                        <option value="<?php echo $gl_get_idb; ?>" selected><?php echo $gl_fetch_dis['judul']; ?></option><?php
                                    }
                                        }
                                        ?>
                                    </select>
			...
			$gl_awal=1;$gl_sub=0;$gl_total=0;
                ...
                @$gl_supplier = strip_tags(trim($_POST['gl_supplier']));
			...
			if (@$gl_id_buku!=''){
				...
				$_SESSION["akhir"][$_SESSION["transaksi"]]=array($gl_judul,$gl_tanggal,$gl_harga_pokok,$gl_jmlh,$gl_sub,$gl_id_buku,$gl_kode,$gl_supplier,$gl_buku_id);
                header("location:config_transaksi_import_short.php?id_buku=$gl_id_buku");
			}
                            ?>
			...
			?>
                            ...
                            <tr>
                                <td colspan="6" align="right" id="paid">
                                    <button type="button" onclick="location.href='config_transaksi_import_short.php?reset=<?php echo $_GET['id_buku']; ?>'" name="reset" id="reset" class="btn btn-danger">Reset</button>
                                    <?php
if(isset($_POST['gl_x_kode'])){
	...
	while($j <= $gl_awal){
        ...
        if($gl_x_kode!="" and $gl_x_id_buku!="" and $gl_x_jmlh!=""){
			$query = mysql_query("insert into gl_detailpasok set id_pasok='$gl_x_kode',id_distributor='$gl_x_supplier', id_buku='$gl_x_id_buku',jml_beli='$gl_x_jmlh',total='$gl_x_totperitem'");
            $gl_sql_updt = mysql_query("update gl_buku set stok=(stok+$gl_x_jmlh),jmlbeli=(jmlbeli+$gl_x_jmlh) where id_buku='$gl_x_id_buku'");
