...
if(isset($_GET['reset'])){
    ...
    header("location:config_transaksi_retur_import.php");
}
...
<html>
    <head>
        ...
        <title>Graylabs - Retur Buku dari Konsumen</title>
        <script>
            var i=1;
            function slide(){
                if(i==1){
    ...
    <body>
        ...
        <div class="full">
                         ...
                         <div class="col-md-2 tip" data-toggle="tooltip" data-placement="bottom"  title="Refresh" id="refresh" onclick="window.location.reload()">
                             <span class="glyphicon glyphicon-refresh"></span>
                                    ...
                                    <div class="col-md-4" id="search" align="center">
                                        <h3>TRANSAKSI RETUR DARI KONSUMEN</h3>
                                    </div>
                                    <div class="col-md-4">
                    ...
                    <div class="col-md-5" style="padding-bottom:20px;border:solid 1px lightgray;margin-bottom:10px;height:300px;">
                        <form target="_self" enctype="multipart/form-data" name="import" id="import" method="post">
                            ...
                            <div class="col-md-12" id="pad-top">
                                ...
                                <div class="col-md-7">
                                     <input type="text" name="gl_kode_beli" id="gl_kode_beli" class="col-md-12" required placeholder="Kode Beli" style="text-transform:uppercase;">
                                </div>
                            </div>
			...
			$gl_awal=1;$gl_sub=0;$gl_total=0;
                ...
                @$gl_id_penjualan = strip_tags(trim($_POST['gl_kode_beli']));
                @$gl_tampil = mysql_fetch_array(mysql_query("select * from gl_buku join gl_detailjual on (gl_buku.id_buku=gl_detailjual.id_buku) where gl_detailjual.id_buku='$gl_id_buku' and gl_detailjual.id_penjualan='$gl_id_penjualan'"));
				@$gl_harga_pokok=@$gl_tampil['total_peritem'];
				@$gl_judul=@$gl_tampil['judul'];
				@$gl_konsumen=@$gl_tampil['nama_konsumen'];
			if (@$gl_id_buku!=''){
				...
				$_SESSION["akhir"][$_SESSION["transaksi"]]=array($gl_judul,$gl_tanggal,$gl_konsumen,$gl_jmlh,$gl_harga_pokok,$gl_sub,$gl_id_buku,$gl_kode,$gl_id_penjualan,$gl_id_penjualan);
                header("location:config_transaksi_retur_import.php");
			}
                            ?>
			...
			?>
                            ...
                            <tr>
                                <td colspan="7" align="right" id="paid">
                                    <button type="button" onclick="location.href='config_transaksi_retur_import.php?reset'" name="reset" id="reset" class="btn btn-danger">Reset</button>
                                    <?php
if(isset($_POST['gl_x_kode'])){
	...
	while($j <= $gl_awal){
        ...
        $gl_x_konsumen = @$_SESSION['akhir'][$j][2];
        $gl_x_jmlh = @$_SESSION['akhir'][$j][3];
        $gl_x_penjualan = @$_SESSION['akhir'][$j][9];
        $gl_x_total = strip_tags(trim($_POST['gl_x_total']));
        if($gl_x_kode!="" and $gl_x_id_buku!="" and $gl_x_jmlh!=""){
			$query = mysql_query("insert into gl_retur set id_retur='$gl_x_kode',id_distributor='-',id_pembelian='$gl_x_penjualan', id_buku='$gl_x_id_buku',id_kasir='$_SESSION[id_kasir]',jumlah='$gl_x_jmlh',tanggal='$gl_x_tanggal',total='$gl_x_total'");
